...
@CompileStatic
class AliOSSResourceReference extends BaseResourceReference {

    protected final static Logger logger = LoggerFactory.getLogger(AliOSSResourceReference.class)
    ...
    @Override ResourceReference createNew(String location) {
        AliOSSResourceReference resRef = new AliOSSResourceReference()
    ...
    @Override List<ResourceReference> getDirectoryEntries() {
        ...
        for (EntityValue child in childList) {
            ...
            dirEntries.add(new AliOSSResourceReference().init(childLoc, child, ecf))
        }
        return dirEntries
    }
    ...
    @Override void putStream(InputStream stream) {
        ...
        if(contentType.startsWith("image/")){
            ...
            if(aliossToolFactory!=null){
                ...
                aliossTool.putObject(ossbucket,filename , stream)
                SerialBlob sblob = new SerialBlob(filename.getBytes(StandardCharsets.UTF_8))
                this.putObject(sblob)
            }else{
    ...
    @Override ResourceReference makeDirectory(String name) {
        ...
        return new AliOSSResourceReference().init("${location}/${name}", ecf)
    }
    @Override ResourceReference makeFile(String name) {
        AliOSSResourceReference newRef = (AliOSSResourceReference) new AliOSSResourceReference().init("${location}/${name}", ecf)
